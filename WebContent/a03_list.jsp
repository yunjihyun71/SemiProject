...
<body>
    ...
    <table cellspacing='0'> <!-- cellspacing='0' is important, must stay -->
        ...
        <c:forEach items="${list}" var="bbs">
       		<form action="scUpdatePage" method="get">
	      		<tr >
		            <td>${bbs.scholar_id}<input  type="hidden" name="scholar_id" value="${ bbs.scholar_id }"/></td>
		            <td>${bbs.scholar_name}</td>
		            <td>${bbs.scholar_money}</td>
		            <td>
		           		<input  type="submit" value="수정"/>
		            </td>
		            <td>
		           		<input type="button" onclick="location.href='scDel?scholar_id=${bbs.scholar_id}'" value="삭제"/>
		            </td>
	       		</tr>
	        </form>
        </c:forEach>
