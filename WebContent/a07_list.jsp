		...
		<table cellspacing='0'> 
			...
			<tbody>
				<c:forEach items="${list}" var="bbs">
       				<form action="gUpdatePage" method="get">
		      			<tr >
				            <td>${bbs.question_id}<input  type="hidden" name="question_id" value="${ bbs.question_id }"/></td>
				            <td>${bbs.question_question}</td>
				            <td>
				           		<input  type="submit" value="수정"/>
				            </td>
				            <td>
				           		<input type="button" onclick="location.href='gDel?question_id=${bbs.question_id}'" value="삭제"/>
				            </td>
		       			</tr>
	       			</form>
				</c:forEach>
				<c:forEach items="${search}" var="search">
	       			<form action="gUpdatePage" method="get">
		      			<tr >
				            <td>${search.question_id}<input  type="hidden" name="question_id" value="${ search.question_id }"/></td>
				            <td> ${search.question_question}</td>
			           		<td>
			           			<input  type="submit" value="수정"/>
			            	</td>
			           		<td>
			           			<input type="button" onclick="location.href='gDel?question_id=${search.question_id}'" value="삭제"/>
			            	</td>
		       			</tr>
	        		</form>
	       		</c:forEach>
